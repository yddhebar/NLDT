...
#extra_string = '_uniform_lead_acc_4vels'
extra_string = ''
...
if env_name == 'CarFollowing':
    ub[0] = env.d_out_of_range

i = 0
while True:
    i += 1
    s = lb + (ub - lb)*np.random.rand(len(lb))
    if env_name == 'CarFollowing':
        s[-1] = np.random.choice(a_range)
...
f_name = agent_name + '_data_' + str(max_data_points) + '_balanced_random'
print(f'Data name = {f_name}')
